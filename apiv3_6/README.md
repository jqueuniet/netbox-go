...
```go
import apiv3_6 "github.com/jqueuniet/netbox-go/apiv3_6"
```
...
For using other server than the one defined on index 0 set context value `apiv3_6.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), apiv3_6.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `apiv3_6.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), apiv3_6.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `apiv3_6.ContextOperationServerIndices` and `apiv3_6.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), apiv3_6.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), apiv3_6.ContextOperationServerVariables, map[string]map[string]string{
...
auth := context.WithValue(
		context.Background(),
		apiv3_6.ContextAPIKeys,
		map[string]apiv3_6.APIKey{
